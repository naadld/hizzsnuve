...
| Gatekeeper | Tên Chốt Chặn | Nền Tảng Thực Thi | Giám Sát Chủ Động (Cloudflare Supervisor) | Quy Tắc Kiểm Soát (Rule) |
| :---: | :--- | :---: | :---: | :--- |
| **GK1** | **Ideation & Outline** | Cloudflare Edge | ☁️ Trực tiếp | • Lọc trùng tuyệt đối với tab `Blacklist`.<br>• Cấu trúc kịch bản đủ 3 Hồi & đúng 15 Phần.<br>• **Khởi tạo ngay 100% Cấu Trúc Thư Mục GDrive** (`01.Preproduction`, `02.Media Generation/audio/keyframes/combined`, `03.Final Production`). |
| **GK2** | **Script Quality & Words** | Cloudflare Edge | ☁️ Trực tiếp | • Độ dài chuẩn **750 – 900 từ** / tập.<br>• **100% chữ số phải đổi thành chữ** (VD: *năm 1945* ➔ *năm một ngàn chín trăm bốn mươi lăm*).<br>• Không chứa nhãn `[NARRATOR]`, `[MUSIC]`.<br>• Đoạn mở đầu Part 1 bắt buộc có câu **"dim the light"**.<br>• **Quy tắc dấu câu & dấu chấm**: Chuẩn hóa `..` thành `.`, chuỗi `....` hoặc `.........` thành `...`, chuẩn hóa khoảng trắng sau dấu câu, loại bỏ hoàn toàn ký tự lạ/mojibake. |
| **GK3** | **Beats & Prompt Formula** | Cloudflare Edge | ☁️ Trực tiếp | • Số lượng beat: Part 1 có 4 ảnh; Parts 2-14 có 4-5 ảnh; Part 15 có 3 ảnh.<br>• Prompt bắt buộc theo chuẩn: *Late-15th-century illuminated manuscript style painting, full-bleed edge-to-edge, zero margins, no frame, 16:9 4K*. |
| **GK4** | **Audio Validator & Auto-Retry** | GitHub Actions | 📡 Giám sát & Auto-Retry | • File WAV sinh ra bắt buộc có **dung lượng $\ge$ 10 KB**.<br>• **Kiểm định âm học chuyên sâu**:<br>  - *Chống file câm/mất tiếng*: Năng lượng RMS $\ge 0.003$, Peak $\ge 0.02$.<br>  - *Chống thiếu chunk/nuốt chữ*: Thời lượng phải tương ứng với số từ ($\ge 0.15s$ / từ).<br>  - *Chống lỗi tensor/méo tiếng*: Không chứa giá trị NaN/Inf.<br>• **Cơ chế Tự Động Thử Lại (Auto-Retry)**: Tự động chạy lại 3 lần ngay trong job nếu vi phạm GK4.<br>• **Cloudflare Giám sát**: Nếu nhận `GK_ERROR` hoặc thiếu Part ➔ Cloudflare tự động re-dispatch GitHub Actions cho đúng Part đó! |
| **GK5** | **Image Physical Validator** | VPS Linux | 📡 Giám sát & Auto-Retry | • Kích hoạt trực tiếp từ Cloudflare Worker sang VPS Listener (không qua trung gian GitHub).<br>• File ảnh tải về bắt buộc có **dung lượng $\ge$ 30 KB** (loại bỏ thumbnail lỗi/rỗng), chuẩn **16:9 4K**.<br>• **Cloudflare Giám sát**: Nếu thiếu ảnh ➔ Cloudflare tự động gọi lại VPS để sinh bù! |
| **GK6** | **Pre-Assembly Asset Auditor** | GitHub Actions | 📡 Giám sát & Kích hoạt | • Kiểm toán trước khi render: Đảm bảo có đủ **15/15 audio parts** và **tối thiểu 15 ảnh keyframe** hợp lệ.<br>• **Cloudflare Giám sát**: Tự động kích hoạt `/assemble` khi nhận đủ tín hiệu từ GK4 & GK5! |
| **GK7** | **Final Video Verification** | GitHub Actions | 📡 Giám sát & Hoàn tất | • Video Master MP4 hoàn thiện (sau khi nối 15 Part + 5s silence) phải có **Duration > 0s** (đủ thời lượng chuẩn ~90 phút).<br>• Âm thanh, hình ảnh đồng bộ 100% trước khi đẩy lên GDrive `03.Final Production/` và đổi Sheet sang `Done`. |
